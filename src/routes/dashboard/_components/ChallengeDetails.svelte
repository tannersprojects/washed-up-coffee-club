...
<div class="space-y-8 py-8">
	<div class="space-y-4">
		...
		<!-- TODO: Handle date formatting and null checks -->
		<div>
			<h3 class="mb-2 font-mono text-xs tracking-widest text-gray-500 uppercase">Start Date</h3>
			<p class="font-mono text-sm text-white">{formatDate(challenge?.startDate || new Date())}</p>
		</div>

		<div>
			<h3 class="mb-2 font-mono text-xs tracking-widest text-gray-500 uppercase">End Date</h3>
			<p class="font-mono text-sm text-white">{formatDate(challenge?.endDate || new Date())}</p>
		</div>
