<script lang="ts">
	...
	const goalDistanceKm = challenge.goalValue ? (challenge.goalValue / 1000).toFixed(1) : null;
</script>

<div class="space-y-8 py-8">
	<div class="space-y-4">
		<h2 class="text-2xl font-bold uppercase text-white">Challenge Details</h2>
		...
		<div>
			...
			<p class="font-mono text-sm uppercase text-white">{challenge.type}</p>
		</div>

		{#if goalDistanceKm}
			<div>
				<h3 class="mb-2 font-mono text-xs tracking-widest text-gray-500 uppercase">Goal Distance</h3>
		...
		<div>
			...
			<p class="font-mono text-sm uppercase text-white">{challenge.status}</p>
		</div>
	</div>
</div>
